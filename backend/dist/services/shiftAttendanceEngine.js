...
// ============================================================================
export function parseShiftTime(timeStr, fallback = '09:00') {
...
export function calculateShiftDateTimes(shiftDateStr, shift) {
...
export function evaluateBackendShiftAttendance(params) {
    ...
    const existingToday = params.attendanceLogs.find((l) => l.employeeId === params.employeeId && (l.shiftDate === todayDateStr || l.date === todayDateStr));
    const startComp = parseShiftTime(shift.startTime, '09:00');
    const endComp = parseShiftTime(shift.endTime, '18:00');
    const earlyComp = parseShiftTime(`${todayTimes.earlyCheckInDateTime.getHours()}:${String(todayTimes.earlyCheckInDateTime.getMinutes()).padStart(2, '0')}`);
    ...
    if (existingToday && existingToday.checkOut) {
        ...
        const nextEarlyComp = parseShiftTime(`${nextTimes.earlyCheckInDateTime.getHours()}:${String(nextTimes.earlyCheckInDateTime.getMinutes()).padStart(2, '0')}`);
...
//# sourceMappingURL=shiftAttendanceEngine.js.map