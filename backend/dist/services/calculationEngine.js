...
export class PayrollConfigurationError extends Error {
    code;
    details;
    constructor(message, code = 'PF_CONFIGURATION_ERROR', details = []) {
        super(message);
 ...
 */
export const computeSalaryStructure = (input) => {
 ...
 * Default PF Rate = 12%
 */
export const calculatePF = (basic, da, conveyance, hra, attendanceBonus, overtime, otherEarnings, pfEnabled, pfRate, wageComponents) => {
    if (!pfEnabled) {
        return {
    ...
    if (pfRate === null || pfRate === undefined || isNaN(pfRate) || pfRate <= 0) {
        throw new PayrollConfigurationError('PF configuration is required to calculate payroll', 'PF_CONFIGURATION_ERROR', ['pf_rate is missing, disabled or invalid while pf_enabled is TRUE']);
    }
    const rate = new Decimal(pfRate);
    let base = new Decimal(0);
    if (wageComponents) {
        if (wageComponents.basic !== false)
            base = base.plus(basic);
        if (wageComponents.da !== false)
            base = base.plus(da);
        if (wageComponents.conveyance !== false)
            base = base.plus(conveyance);
        if (wageComponents.hra === true)
            base = base.plus(hra);
        if (wageComponents.attendance_bonus === true)
            base = base.plus(attendanceBonus);
        if (wageComponents.overtime === true)
            base = base.plus(overtime);
        if (wageComponents.other_earnings === true)
            base = base.plus(otherEarnings);
    }
    else {
 ...
 */
export const calculateESIC = (basic, da, conveyance, hra, attendanceBonus, overtime, otherEarnings, grossSalary, esicEnabled, esicRate, threshold, wageComponents) => {
    if (!esicEnabled) {
        return {
    ...
    if (wageComponents) {
        ...
        if (wageComponents.conveyance !== false)
            base = base.plus(conveyance);
        if (wageComponents.hra !== false)
            base = base.plus(hra);
        if (wageComponents.attendance_bonus !== false)
            base = base.plus(attendanceBonus);
        if (wageComponents.overtime !== false)
            base = base.plus(overtime);
        if (wageComponents.other_earnings !== false)
            base = base.plus(otherEarnings);
    }
    else {
 ...
 */
export const calculateLOP = (monthlySalary, workingDays, lopDays) => {
 ...
 */
export const computeFullPayroll = (input) => {
    ...
    const pfResult = calculatePF(basic, da, conveyance, hra, attBonus, otAmount, otherEarnings, pfEnabled, input.settings.pfRate, input.settings.pfWageComponents);
    // 4. Statutory Deductions (ESIC)
    const esicResult = calculateESIC(basic, da, conveyance, hra, attBonus, otAmount, otherEarnings, grossSalary, esicEnabled, input.settings.esicRate, input.settings.esicSalaryThreshold, input.settings.esicWageComponents);
