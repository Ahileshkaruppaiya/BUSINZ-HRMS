...
export class PasswordResetRepository {
     ...
     */
    async countRecentRequests(employeeId, windowMinutes = 15) {
        ...
        return inMemoryResets.filter((r) => r.employeeId.toLowerCase() === cleanId && r.createdAt >= cutoff).length;
    }
    /**
     ...
     */
    async getLatestActiveReset(employeeId) {
        const cleanId = employeeId.trim().toLowerCase();
        if (isRealSupabaseConfigured()) {
            try {
                const supabase = getSupabaseAdmin();
     ...
     */
    generateResetToken(employeeId, email) {
        return jwt.sign({
            sub: employeeId,
            email,
