...
export class AttendanceRepository {
    async getAttendanceLogs(filters) {
        ...
        if (filters?.date) {
            results = results.filter((r) => r.date === filters.date || r.shiftDate === filters.date);
        }
        if (filters?.employeeId) {
    ...
    async getShiftAttendanceStatus(employeeId, now) {
        ...
        if (!assigned && emp?.workShift) {
            assigned = shifts.find((s) => s.shiftName.toLowerCase().includes(emp.workShift.toLowerCase()) || s.id === emp.workShift);
        }
        if (!assigned) {
            assigned = shifts[0] || {
                id: 'SH-01',
    ...
    async recordPunch(punch) {
        ...
        // Evaluate shift window state using punch time
        ...
        let existing = inMemoryAttendance.find((r) => r.employeeId === punch.employeeId && (r.shiftDate === targetShiftDate || r.date === targetShiftDate));
        if (!existing) {
            existing = {
                id: `att-${Date.now()}`,
                ...
                locationAddress: punch.locationAddress || 'Main Corporate Office',
            };
            ...
            if (!existing.shiftId)
                existing.shiftId = assignedShiftId;
            if (!existing.shiftDate)
                existing.shiftDate = targetShiftDate;
        }
        if (isRealSupabaseConfigured()) {
            try {
                const supabase = getSupabaseAdmin();
    ...
    async verifyFace(employeeId, _facePhotoBase64) {
