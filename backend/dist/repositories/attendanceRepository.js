        ...
        if (isRealSupabaseConfigured()) {
            try {
                ...
                const { data: csData } = await supabase
                    ...
                    .eq('setting_key', 'attendance_records_data')
                    .maybeSingle();
                let allRecords = Array.isArray(csData?.setting_val) ? csData.setting_val : [];
                const idx = allRecords.findIndex((r) => (r.employeeId === punch.employeeId || r.employee_id === punch.employeeId) &&
