...
export const SPECIAL_PERMISSIONS_CATALOG: SpecialPermissionItem[] = [
  // Payroll & Salary
  ...
  // Advance Salary
  { key: 'approve_salary_advance', label: 'Approve Advance Salary', category: 'Advance Salary', description: 'Authorize loan & advance applications up to policy cap' },
...
export class RbacRepository {
  ...
  private modules: ModuleDefinition[] = [...CORE_MODULES];

  private permissionsByRole: Record<string, RolePermissionConfig> = {};
  ...
  public getAllRBACData(): RBACDataResponse {
    return {
      roles: [...this.roles],
  ...
  public getRoleByName(name: string): RoleDefinition | undefined {
    return this.roles.find(r => r.name.toLowerCase() === name.toLowerCase());
  }

  public getRolePermissions(roleIdOrName: string): RolePermissionConfig | undefined {
  ...
  }, changedBy: string = 'Admin'): RoleDefinition {
  ...
  public updateRole(id: string, updates: {
    name?: string;
    description?: string;
    department?: string;
    status?: 'Active' | 'Inactive';
  ...
  public duplicateRole(id: string, newName: string, changedBy: string = 'Admin'): RoleDefinition {
  ...
  public deleteRole(id: string, changedBy: string = 'Admin'): boolean {
  ...
  public saveRolePermissions(roleId: string, updates: {
    dataAccess: DataAccessLevel;
    modulePermissions: Record<string, PermissionAction[]>;
    specialPermissions: string[];
  }, changedBy: string = 'Admin'): RolePermissionConfig {
  ...
  public addDynamicModule(moduleData: {
    key: string;
    name: string;
    category?: string;
    description?: string;
  }, changedBy: string = 'Admin'): ModuleDefinition {
